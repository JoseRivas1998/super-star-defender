...
public class HUD {
	...
	public void update(int scoreint, int enemiesKilled, int enemiesNeeded, String gameOver) {
	...
	public void draw(SpriteBatch sb, ShapeRenderer sr, MyCamera cam, Player p, boolean won, boolean highScoreBeat) {
		sb.begin();
		sb.setProjectionMatrix(cam.combined);
		if(won || highScoreBeat) Game.res.getFont("small").setColor(Color.WHITE);
		...
		if(highScoreBeat) Game.res.getFont("small").setColor(MyConstants.randomColor());
		Game.res.getFont("small").draw(sb, score, sX, sY);
		if(highScoreBeat) Game.res.getFont("small").setColor(MyConstants.randomColor());
		Game.res.getFont("small").draw(sb, highScore, hsX, hsY);
		if(highScoreBeat) Game.res.getFont("small").setColor(Color.WHITE);
